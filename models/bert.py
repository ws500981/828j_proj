...
def prepare_test_data(df, bert_name):
...
class BertModel(object):
    def __init__(self,train_loader, val_loader, test_loader, bert_name):
        super(BertModel, self).__init__()
        self.train_loader = train_loader
        self.val_loader = val_loader
        self.test_loader = test_loader
        self.model = BertForSequenceClassification.from_pretrained(bert_name, num_labels=2)
...
if __name__ == "__main__":
    ...
    test_df = pd.read_csv(os.path.join("./../data/fin_data/","test_lort.csv"), dtype=object)
    test_df["label"] = test_df["label"].astype(int)
    
    bert_name = 'bert-large-uncased'

    train_encodings, train_labels, val_encodings, val_labels = prepare_dataset(df, bert_name)
    test_encodings, test_labels = prepare_test_data(test_df, bert_name)
    ...
    obj = BertModel(train_loader, val_loader, test_loader, bert_name)
    obj.train()
